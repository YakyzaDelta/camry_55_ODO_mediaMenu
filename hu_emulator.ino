/*
  ============================================================
  Эмулятор головного устройства (ГУ) для приборной панели (IPC)
  ESP32 + MCP2515   —   версия с диагностикой через Serial

  ЧТО БЫЛО ИСПРАВЛЕНО по сравнению с первой версией:
   1) БАГ в setup(): `while (!CAN.begin(...) == CAN_OK)` - из-за
      приоритета операторов это читалось не так, как задумано
      (сначала !CAN.begin(...), потом сравнение с CAN_OK). По факту
      значение CAN_OK=0 спасало от неправильной работы, но выглядело
      как баг и маскировало реальный код ошибки инициализации -
      теперь код ошибки печатается в Serial явным текстом.
   2) sendMsgBuf() у вас нигде не проверялся - т.е. если кадр не
      уходил (нет ACK от другого узла, шина занята, TX-буфер занят) -
      вы об этом просто не узнавали. Теперь каждый неудачный кадр
      считается и раз в 2 секунды выводится статистика.
   3) Добавлен режим SNIFF (команда в Serial-мониторе: 's') - слушает
      шину и печатает входящие кадры. Это позволяет ПРОВЕРИТЬ, что
      скорость/проводка верны, ДО того как пытаться слать что-то от
      имени ГУ (раз ваш "Silent Listener" на этой же скорости кадры
      ловил - если этот же ESP32 их тоже ловит, значит speed/wiring ок,
      и проблема именно в передаче/ACK).
   4) Добавлен режим VERBOSE ('v') - печатает каждый отправляемый кадр.

  !!! ПРОВЕРЬТЕ ПЕРЕД ПРОШИВКОЙ !!!
   - CAN_SPEED / MCP_CLOCK - должны СОВПАДАТЬ с тем, что было в вашем
     "CAN Silent Listener" скетче (там уже подобрано рабочее значение -
     возьмите его оттуда дословно, не гадайте).
   - CS_PIN - пин Chip Select MCP2515.
   - На шине ОБЯЗАТЕЛЬНО должен быть второй живой узел (настоящая
     приборка, запитанная и не спящая) и штатная терминация 120 Ом.
     Если ESP32 - единственный узел на шине без ACK от кого-либо,
     контроллер MCP2515 будет уходить в error/bus-off и ничего слать
     не сможет физически - это САМАЯ частая причина "не работает"
     в таких самодельных эмуляторах.
  ============================================================
*/
